    ...
    if (isset($_COOKIE['dlike_username']) || $_COOKIE['dlike_username'])
    {
        ...
        if ($check_bot_likes->num_rows >= 10){die(json_encode(['error' => true, 'message' => 'Phew ...You can not do more likes!']));}
            ...
            if (mysqli_query($conn, $sqlm))
            {
                ...
                $reward_status = '0';
                $sql_upvotes = $conn->query("INSERT INTO dlike_upvotes (curator, author, permlink, ip_addr, status, curation_time) VALUES ('".$userval."', '".$author."', '".$permlink."', '".$thisip."', '".$reward_status."', 'now()')");
