...
if (isset($_POST['action']) && $_POST['action'] =='shares_limit' && isset($_POST['user']) && $_POST['user'] != '')
{
    ...
    if (empty($errors)) {
        ...
        if ($sql_post_limit->num_rows >= 5) {die(json_encode(['error' => true, 'message' => 'Phew ... You reached max daily share limit!']));} else {die(json_encode(['error' => false, 'message' => 'You can post!']));}
...
if (isset($_POST['action']) && $_POST['action']=='unique_post' && isset($_POST['newurl']) && $_POST['newurl'] !='') {
    ...
    if ($sql_unique_url->num_rows > 0) {die(json_encode(['error' => true, 'message' => 'URL already shared. Can not be shared again!']));}else{die(json_encode(['error' => false, 'message' => 'Url is unique']));}
}
?>