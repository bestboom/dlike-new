...
if (isset($_POST['name_profile']) && $_POST['name_profile'] != '') {
	...
	if($p_img !=""){if(!is_array(getimagesize($p_img))){$errors = "Profile image link does not seems a valid image file!";}}
	if($p_cover_img !=""){if(!is_array(getimagesize($p_cover_img))){$errors = "Cover image link does not seems a valid image file!";}}
