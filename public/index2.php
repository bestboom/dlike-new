...
if ($sql_T && $sql_T->num_rows > 0){  while ($row_T = $sql_T->fetch_assoc()){
    ...
    $post_tags = trim($row_T["tags"]); if(strpos($post_tags, " ") !== false){ $post_tags = array_unique(explode(" ",$post_tags));	foreach($post_tags as $tag) {echo $post_tags= '<a href="https://dlike.io/'.$tag.'">#'.$tag.'</a>'.' ';}return $post_tags;}else{ $post_tags= '<a href="https://dlike.io/'.$post_tags.'">#'.$post_tags.'</a>';}
