$(document).ready(function(){	
	...
	steem.api.getDiscussionsByCreated(query, function (err, res) { res.forEach(($post, i) => {
		...
		if(metadata && metadata.community == "dlike"){
    		...
    		$.ajax({type: "POST",url: '/helper/steem/getuserpoststatus.php',data:{'author':author},dataType: 'json',success: function(response) {console.log(response);var mylabel = permlink +author;var newValue = mylabel.replace('.', '');if(response.status == "OK") {var all_status = response.setstatus;if(all_status == "3") {var colorset = 'red';$('.userstatus_icon' + newValue).css({"color": colorset});var erroset = "PRO User";}$('.userstatus_icon' + newValue).hover(function() {toastr.success(erroset);});	}else {$('.userstatus_icon' + newValue).remove();}}});
