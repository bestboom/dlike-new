		...
		res.forEach(($post, i) => {
			...
			if(metadata && metadata.community == "dlike"){
				...
				$(content).append('<div class="col-lg-4 col-md-6 postsMainDiv mainDiv'+ currentLikesDivElement +'" postLikes="0" postNumber="'+ currentPostNumber +'">\n' +
					...
					'\n' +
					'<div class="hov-wrap"><figure class="hov-item"><img src="./images/post/dlike-hover.png" alt="img" class="img-responsive"><span class="hov_me" data-toggle="modal" data-target="" data-permlink="' + $post.permlink + '" data-author="' + $post.author + '"><figcaption class="hov-title"><h5><span id="hov-num" class="commentsDiv' + currentLikesDivElement + '">0</span></h5></figcaption></span></figure></div></div>\n' +
					'\n' +
					...
					'</div>\n' +
					'<div class="post-comments"><a class="upvoting" data-toggle="modal" data-target="#upvoteModal" data-permlink="' + $post.permlink + '" data-author="' + $post.author + '"><i class="fas fa-chevron-circle-up"></i></a><span>&nbsp; | ' + $post.active_votes.length + ' Votes</span></div>\n' +
					'</div>\n' +
					'</div>\n' +
				'</article></div>');
