$(document).ready(function(){	
	...
	steem.api.getDiscussionsByCreated(query, function (err, res) { res.forEach(($post, i) => {
		...
		if(metadata && metadata.community == "dlike"){
    		...
    		//user-pro status
			$.ajax({type:"POST",url:"/helper/steem/getuserpoststatus.php",data:{author:author},dataType:"json",success:function(s){var t=(permlink+author).replace(".","");if("OK" 	==s.status){ console.log(s.status)if("3"==s.setstatus){$(".userstatus_icon"+t).css({color:"red"});var e="PRO User"}$(".userstatus_icon"+t).hover(function(){toastr.success(e)})}else $(".userstatus_icon"+t).remove()}});
